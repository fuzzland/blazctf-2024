...
contract CurvePriceOracle is IPriceOracle {
    ...
    constructor(address _curvePool, uint256 _idx, uint256 anchor) {
        ...
        uint256 absDiff = 0;
        if (ICurvePool(curvePool).price_oracle(_idx) > anchor) {
            absDiff = ICurvePool(curvePool).price_oracle(_idx) - anchor;
        } else {
            absDiff = anchor - ICurvePool(curvePool).price_oracle(_idx);
        }
        require(absDiff <= 1e8, "Price oracle has been manipulated :(");
    }

    function getAssetPrice(uint256 _assetId) external view returns (uint256) {
        return ICurvePool(curvePool).price_oracle(idx);
    }

    function getSpotPrice() external view returns (uint256) {
        return ICurvePool(curvePool).last_price(idx);
    }
}
...
contract TonyLend is ReentrancyGuard, Ownable {
    ...
    function borrow(uint256 _assetId, uint256 _amount) external nonReentrant {
        ...
        uint256 healthFactor = calculateHealthFactor(msg.sender);
        require(healthFactor >= PRECISION, "Borrow would result in undercollateralization");
    ...
    function repay(uint256 _assetId, uint256 _amount) external nonReentrant {
        require(_assetId < assetCount, "Invalid asset");
        require(_amount > 0, "Amount must be greater than 0");
        ...
        uint256 borrowedAmount = account.borrowed[_assetId];
        uint256 repayAmount = Math.min(_amount, borrowedAmount);

        require(asset.token.transferFrom(msg.sender, address(this), repayAmount), "Transfer failed");

        account.borrowed[_assetId] -= repayAmount;
        asset.totalBorrowed -= repayAmount;

        emit Repay(msg.sender, _assetId, repayAmount);
    }
