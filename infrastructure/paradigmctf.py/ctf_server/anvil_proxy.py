...
ALLOWED_NAMESPACES = ["web3", "eth", "net", "blaz"]
DISALLOWED_METHODS = [
    "eth_sign",
    "eth_signTransaction",
    "eth_signTypedData",
...
def validate_request(request: Any) -> Optional[Dict]:
    ...
    if (
        not ((request_method in ALLOWED_METHODS
             or request_method.split("_")[0] in ALLOWED_NAMESPACES)
             and request_method not in DISALLOWED_METHODS)
    ):
