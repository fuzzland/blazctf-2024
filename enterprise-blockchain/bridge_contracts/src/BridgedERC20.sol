...
contract FlagToken is ERC20 {
    constructor(address bridge, address user) ERC20 ("FLAG Token", "FTT") {
        _mint(bridge, 100000000000 ether);
        _mint(user, 1 ether);
    }
}
...
contract BridgedERC20 is ERC20 {
    ...
    modifier onlyRemoteBridge() {
        ...
        require(BRIDGE.relayedMessageSenderChainId() != 0 && BRIDGE.remoteBridgeChainId(BRIDGE.relayedMessageSenderAddress()) == BRIDGE.relayedMessageSenderChainId(), "RB2");
        _;
    }
