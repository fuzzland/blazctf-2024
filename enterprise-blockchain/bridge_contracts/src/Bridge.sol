...
contract Bridge {
    ...
    modifier onlyRemoteBridge() {
        require(msg.sender == address(this) &&
                ...
                remoteBridgeChainId[relayedMessageSenderAddress] == relayedMessageSenderChainId, "RB");
        _;
    }
    ...
    function ERC20Register(address _remoteToken, string memory _name, string memory _symbol) external onlyRemoteBridge {
    ...
    function sendRemoteMessage(uint256 _targetChainId, address _targetAddress, bytes calldata _message) public payable {
        require(_targetChainId != block.chainid, "C");
        require(_targetAddress != address(0), "A");
        require(msg.sender == address(this), "M");
        emit SendRemoteMessage(
            _targetChainId,
            _targetAddress, 
            msg.sender, 
            msg.value,
