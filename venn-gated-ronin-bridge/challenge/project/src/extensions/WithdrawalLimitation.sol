...
import "../onchain-firewall/contracts/FirewallConsumer.sol";

abstract contract WithdrawalLimitation is GatewayV3, FirewallConsumer {
   ...
   *
   */
  function setThreshold(uint256 num, uint256 denom) external virtual override onlyProxyAdmin firewallProtected {
  ...
  ) external virtual onlyProxyAdmin firewallProtected returns (uint256 _previousNum, uint256 _previousDenom) {
   ...
   *
   */
  function setHighTierThresholds(address[] calldata _tokens, uint256[] calldata _thresholds) external virtual onlyProxyAdmin firewallProtected {
   ...
   *
   */
  function setLockedThresholds(address[] calldata _tokens, uint256[] calldata _thresholds) external virtual onlyProxyAdmin firewallProtected {
    if (_tokens.length == 0) revert ErrEmptyArray();
   ...
   *
   */
  function setUnlockFeePercentages(address[] calldata _tokens, uint256[] calldata _percentages) external virtual onlyProxyAdmin firewallProtected {
    if (_tokens.length == 0) revert ErrEmptyArray();
   ...
   *
   */
  function setDailyWithdrawalLimits(address[] calldata _tokens, uint256[] calldata _limits) external virtual onlyProxyAdmin firewallProtected {
    if (_tokens.length == 0) revert ErrEmptyArray();
